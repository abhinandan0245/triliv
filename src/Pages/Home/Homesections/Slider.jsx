...
const Slider = () => {
  ...
  const animateSlide = (slideElement) => {
    ...
    Object.values(elements).forEach((el) => {
      if (el) {
        el.classList.remove(
          "animate__animated",
          ...Object.values(el.dataset.animations || {})
        );
      }
    });
    ...
    setTimeout(() => {
      if (elements.title) {
        elements.title.classList.add(
          "animate__animated",
          slides[slideElement.dataset.swiperSlideIndex]?.animations.title
        );
      }
    }, 100);
    ...
    setTimeout(() => {
      if (elements.subtitle) {
        elements.subtitle.classList.add(
          "animate__animated",
          slides[slideElement.dataset.swiperSlideIndex]?.animations.subtitle
        );
      }
    }, 300);
    ...
    setTimeout(() => {
      if (elements.button) {
        elements.button.classList.add(
          "animate__animated",
          slides[slideElement.dataset.swiperSlideIndex]?.animations.button
        );
      }
    }, 500);
  };
  ...
  return (
    <div className="tf-slideshow slider-plant slider-default">
      <div
        dir="ltr"
        className="swiper tf-sw-slideshow slider-effect-fade swiper-initialized swiper-horizontal swiper-backface-hidden"
        data-preview="1"
        data-tablet="1"
        data-mobile="1"
        data-centered="false"
                      ...
                      <div className="col-12">
                        <div className="content-slider text-center">
                          <div className="box-title-slider">
                            <div
                              className="heading font-7 display-2xl text-white fw-semibold"
                              data-animations={slide.animations.title}
                            >
                              {slide.title}
                            </div>
                            <p
                              className="sub text-md text-white"
